...
public:
    ...
    void file() {
        ...
        fout << "Фамилия:" << n << endl;
        fout << "Опыт:" << worker1.getE() << endl;
        fout << "Часовая заработная плата:" << worker1.getW() << endl;
        fout << "Количество отработанных часов:" << worker1.getH() << endl;
        fout << "Заработная плата:" << worker1.salcount() << endl;
        fout << "Премия:" << worker1.premcount() << endl;
...
int main()
{
    ...
    //почему-то при выводе фамилии кириллицей ломается кодировка
    worker worker1;
    string n;
    int e, w, h;
    cout << "Фамилия:";
